...
def run(playwright: Playwright) -> None:
    ...
    page.locator("body").press("Alt+s+4"),

    elements = [
        # Начиная с 9 строчки копируем
        page.locator("div").filter(has_text=re.compile(r"^Лаборатория\(Биология\)$")),
        page.get_by_role("navigation").get_by_text("Датчик T эксп."),
        page.get_by_text("Датчик pH"),
        page.locator("div").filter(has_text=re.compile(r"^Датчик влажности$")),
